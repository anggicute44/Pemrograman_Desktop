...
Public Class Form1
    ...
    Private Sub GetFiles(ByVal PathName As String)
        ...
        LlvImage.Items.Clear()
        LlvImage.BeginUpdate()
...
End With
                LlvImage.Items.Add(LItem)
            End If
Next
        LlvImage.EndUpdate()
        LlvImage.Columns(0).Width = -2
    End Sub
    ...
    Private Sub lvImage_SelectedIndexChanged(sender As Object, e As EventArgs) Handles LlvImage.SelectedIndexChanged
        Dim _exif As Info
        LlvExif.Items.Clear()
        If LlvImage.SelectedItems.Count > 0 Then
            PictureBox1.Image = Bitmap.FromFile(LlvImage.SelectedItems(0).Tag.ToString)
            ToolStripStatusReady.Text = LlvImage.SelectedItems(0).Tag.ToString
            ...
            For Each propertyName As String In _exif.PropertyItems.Keys
                Try
                    ...
                    LlvExif.Items.Add(LItem)
                Catch eks As Exception
                End Try
            Next
        End If
        LlvImage.Columns(0).Width = -2
        LlvExif.Columns(0).Width = -2
    End Sub

    Private Sub lvImage_DoubleClick(sender As Object, e As EventArgs) Handles LlvImage.DoubleClick
        Process.Start(LlvImage.SelectedItems(0).Tag)
    End Sub
